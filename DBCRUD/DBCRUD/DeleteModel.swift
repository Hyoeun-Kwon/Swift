...
class DeleteModel{
    ...
    var urlPath = "http://192.168.237.130:8080/ios/studentDelete_ios.jsp" // + urlAdd ( ?와 & 로 쭉쭉 이어갈 거임)
    
    func deleteItems(code: String) -> Bool { //data를 달고 넘어와서 DB연결해야함
        // Error message를 주기위해 Bool ( 잘됏다 아니다!)
        var result: Bool = true
        let urlAdd = "?code=\(code)" // 띄어쓰기하면 안됨!
